<template>
  <div class="layout">
    <div style="position: absolute; top: 10px; right: 10px; z-index: 999; ">
      <button density="compact" icon="mdi-plus" :class="{active: !isEditMode}" @click="doToggleDrawer(true)">#</button>
    </div>
    <div
      v-show="isEditMode"
    ...
    >
      ...
      <div>
        <button class="btn" :class="{active: !isEditMode}" @click="doToggleDrawer(false)">close</button> 
      </div> 
...
import {GridLayout, GridItem} from "../lib-grid-layout"
...
//const emit = defineEmits<{ (event: "update-description", description: string): void }>();
const emit = defineEmits<{ (event: "update-widget", widgets: any[]): void }>();
